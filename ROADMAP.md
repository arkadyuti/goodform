...
- ⬜ Block completion → reassessment → next block toward 5K/10K (FR-2.6, v2.0)
- ⬜ Session + weekly check-in reminders, no-guilt policy (FR-9.x)
- ⬜ Supplement checklist + dietary pattern guidance (FR-6.4, 6.5)
- ⬜ Pre/post-session fuelling tied to session times (FR-6.6)
- ⬜ Nutrition guardrails: disordered-pattern detection → withdraw targets (FR-6.7, SR-5)
- ⬜ GDPR self-serve: account deletion + full data export (NFR-10, NFR-12)
