      ...
      <div className="flex flex-col gap-5">
        <div>
          ...
          <p className="mt-2 leading-relaxed text-ink-soft">
            Put your shoes on and take your phone outside. This takes about ten minutes and there is nothing to
            get right.
          </p>
        </div>
        ...
        <ol className="flex flex-col gap-3">
          ...
          <Step number={2} title="Then run — as slowly as you like">
            Slow enough that you could talk to someone. There's no distance to reach and no clock to beat.
          </Step>
          <Step number={3} title="Stop whenever you want to">
            Not when you can't go on — when you'd rather stop. One minute is a perfectly normal answer.
          </Step>
        </ol>

        <Note tone="run">
          Whatever number comes out of this decides where your plan starts. A small number means a plan that
          fits you, which is the whole point.
        </Note>
  ...
  if (stage === 'walk' || stage === 'run') {
    ...
    return (
      ...
      >
        ...
        <div className={`flex flex-1 flex-col items-center justify-center px-6 ${isWalk ? 'text-ink' : 'text-white'}`}>
          <p
          ...
          <p className="tabular mt-1 text-[clamp(5rem,30vw,9rem)] leading-[0.85]" style={{ fontWeight: 800 }}>
            {formatClock(isWalk ? WARMUP_SEC - elapsed : elapsed)}
          </p>
          <p className={`mt-4 max-w-xs text-center text-[1.0625rem] ${isWalk ? 'text-ink/80' : 'text-white/85'}`}>
            {isWalk
    ...
    <div className="flex flex-col gap-5">
      <div>
        ...
        <h1 className="mt-1.5 text-4xl" style={{ fontWeight: 780 }}>
          You ran {minutes < 0.5 ? 'under half a minute' : `${minutes} ${minutes === 1 ? 'minute' : 'minutes'}`}
        </h1>
        <p className="mt-2 leading-relaxed text-ink-soft">
...
export function StopReasonChoice({
  ...
  return (
    <Choices
      value={value ? [value] : []}
      onChange={([v]) => v && onPick(v as StopReason)}
      options={[
        {
          value: 'breath',
